    ...
    public partial class JiaoYan : Form
    {
        ...
        private void fenxi()
        {
            ...
            if (DIS[7 - qiehuanfa1] + "" == "0")
            {
                ...
                liangcheng = 1;
                qiehuastate = true;
                if (step==0&&QIEHUATIME==-1)
                {
                    QIEHUATIME=GetTimeStamp(DateTime.Now);
            ...
            if (DIS[7 - famao] + "" == "0")
            {
                ...
                if (chuchi == 1&&step==2)
                {
                    if (debug == 1)
                    {
                        ff.ShowInfoTip("离线=初次校验，安装阀帽得分");
                    }
                    //   
                    //chaixiefamao();
                    log1.updatelog("离线=初次校验，安装阀帽得分" + azfm, "lx-azfm-2", azfm, datahelp.QId);
            ...
            if (DIS[7 - xieya] + "" == "0")
            {
                ...
                if (step == 1)
                {
                    if (debug == 1)
                    {
                        dishow("离线=第一次泄压阀打开得分" + dkxyf1);

                    }
                    log1.updatelog("离线=第一次次泄压阀打开得分" + dkxyf1, "lx-xyfkq-1", dkxyf1, datahelp.QId);
                ...
                if (step == 2)
                {
                    if (debug == 1)
                    {
                        dishow("离线=第二次泄压阀打开得分" + dkxyf2);

                    }
                    log1.updatelog("离线=第二次泄压阀打开得分" + dkxyf2, "lx-xyfkq-2", dkxyf2, datahelp.QId);
                ...
                if (step == 3)
                {
                    if (debug == 1)
                    {
                        dishow("离线=第三次泄压阀打开得分" + dkxyf3);

                    }
                    log1.updatelog("离线=第三次泄压阀打开得分" + dkxyf3, "lx-xyfkq-3", dkxyf3, datahelp.QId);
                ...
                if (step == 4)
                {
                    if (debug == 1)
                    {
                        dishow("离线=保压测试泄压阀打开得分" + dkxyf4);

                    }
                    log1.updatelog("离线=保压测试泄压阀打开得分" + dkxyf4, "lx-xyfkq-4", dkxyf4, datahelp.QId);
            ...
            else
            {
                ...
                if (step == -1)
                {
                    ...
                    log1.updatelog("离线=校验前，泄压阀关闭得分" + xygb, "lx-xyfgb", xygb, datahelp.QId);
        ...
        {/*&&(QIEHUATIME-JYTIME)<0*/
            ...
            {
               // MessageBox.Show(JYTIME+"DF"+GetTimeStamp(DateTime.Now));
                ...
                Action x = () =>
                {
                    ...
                    ProjectInfo p = new ProjectInfo("0");
                    p.Show();
                };
                this.Invoke(x);
                return;
        ...
        private void foujue2()
        {
            if (foujuestate2==false)
            {
                ...
                log1.updatelog("严重错误！调节螺母拧错", "lx-fj2", 0, datahelp.QId);
                ff.ShowErrorDialog("严重错误！调节螺母拧错,当前考试结束，得分为0");
                g.updateGrade(0, "lx-total", datahelp.QId);
        ...
        public int SJLMSTAT = 0;
        public bool JiaoYanDONGL = false;
        public int JYTIME = -1;
        public int QIEHUATIME = -1;
        ...
        private void voldetla1(string v, string sb1, string t1)
        {
            ...
            Action tongdao = () =>
            {
                ...
                if (step==0)
                {
                    ...
                    //  }
                    if (xieyastate==false&&Math.Abs(a1-sjdwq)>100)
                    {
                        ff.ShowInfoTip("锁紧螺母动了");
                        foujue();
                    }
                    if (ap.f0 > ap.f1&&Math.Abs(a1-sjdwq)>100)
                    {
                        ff.ShowInfoTip("锁紧螺母动了");
                        //泄压阀未开，调螺母，因为调螺母在初次之间,第一次开启前，定义初次开启泄压阀变量为必要条件xiedakai=0
        ...
        private void voldetla(string sb1, string t8)
        {
            ...
            int b = Convert.ToInt32(t8.ToString(), 16);
             if (debug==1)
            {
            ff.ShowInfoTip("校验阀实时"+current+"校验阀变化"+Math.Abs(dwqcha-a));
            }



            //if (Math.Abs(dwqcha-a)>500&&dwqcha!=a&&step==0)
            //{

            //    ff.ShowInfoTip("校验阀动了");

            //    JiaoYanDONGL=true;
            //    //   记录当前时间戳

            //    JYTIME=GetTimeStamp(DateTime.Now);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            switch (step)
            {
                ...
                case 1:
                    ...
                    maz = ap.maz;
                  // MessageBox.Show(""+maz+"err"+mazerr);
                    maz90 = ap.maz90;
                    ta = 60;
                    if (SJLMSTAT==-1)
                    {
...

        {
            ...
            if (DIS == "11111001" || DIS == "01111001" || DIS == "11111000" || DIS == "01111000")
            {
                if (last==false&&famaostate==true)
                {
                    Thread a1 = new Thread(shot1);
                    a1.Start();
                }
